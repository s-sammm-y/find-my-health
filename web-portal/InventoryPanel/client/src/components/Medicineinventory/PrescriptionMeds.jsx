...
const PrescriptionMeds=()=>{
    const [prescriptionInventory,setPrescriptionInventory] = useState([]); 
    ...
    useEffect(()=>{
        const fetchInventory=async()=>{
            try{
                const prescription = await axios.get('http://localhost:3003/get-medicine-inventory',{
                    params:{med_type:'prescription'}
                })
                const res = prescription.data.data;
                setPrescriptionInventory(res)
            }catch(error){
    ...
    return(
        ...
        <div className="min-h-screen overflow-auto">
            {prescriptionInventory.map((item,index)=>(
